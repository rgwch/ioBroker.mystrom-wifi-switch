...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Gerry Weirich';
    var email     = grunt.option('email')  || 'weirich@elexis.ch';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new mystrom name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/mystrom.png')) {
            fs.renameSync(__dirname + '/admin/mystrom.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/mystrom.html')) {
            fs.renameSync(__dirname + '/widgets/mystrom.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/mystrom/js/mystrom.js')) {
            fs.renameSync(__dirname + '/widgets/mystrom/js/mystrom.js', __dirname + '/widgets/mystrom/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/mystrom')) {
            fs.renameSync(__dirname + '/widgets/mystrom',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Mystrom/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Mystrom'
                        },
                        {
                            match: /Gerry Weirich/g,
                            replacement: author
                        },
                        {
                            match: /weirich@elexis.ch/g,
                            replacement: email
                        }
                    ]
                },
